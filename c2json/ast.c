...
char *ast_type_to_str(AST_NODE_TYPE type)
{
    ...
    case DirectDeclaratorBrackets:
        return "DirectDeclaratorBrackets";
    ...
    case DirectAbstractDeclaratorBrackets:
        return "DirectAbstractDeclaratorBrackets";
    case DirectAbstractDeclaratorParen:
        return "DirectAbstractDeclaratorParen";
    case Initializer:
        return "Initializer";
    case InitializerList:
        return "InitializerList";
    case InitializerListElem:
        return "InitializerListElem";
    case DesignatorList:
...
void ast_free(AST_NODE *root)
{
    if (root == NULL)
        return;
    if (root->type == Identifier || root->type == IntegerConstant || root->type == FloatingConstant || root->type == CharacterConstant || root->type == StringLiteral)
    {
...
char *ast_to_json(AST_NODE *root, int shift, char *tab, char *(*cont_to_str)(AST_NODE *))
{
    ...
    size_t json_size = strlen(type_str) + strlen(content_str) + strlen(children_num_str) + strlen(children_str) + strlen(tab) * (shift * 6 + 4) + sizeof(char) * (62 + 1);
